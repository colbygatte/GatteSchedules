...
class DB {
    ...
    static func getSettings(completion: @escaping (FIRDataSnapshot) -> Void) {
    ...
    // Days
    //
    static func get(day: Date, completion: @escaping (FIRDataSnapshot) -> Void) {
    ...
    static func getUsers(completion: @escaping (FIRDataSnapshot) -> Void) {
        DB.usersRef.queryOrdered(byChild: "teamid").queryEqual(toValue: DB.teamid).observeSingleEvent(of: .value, with: { snap in
            completion(snap)
        })
    }

    static func getUserData(uid: String, completion: @escaping (FIRDataSnapshot) -> Void) {
