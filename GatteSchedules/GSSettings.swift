...
class GSSettings: NSObject {
    ...
    init(snapshot: FIRDataSnapshot) {
        ...
        guard let settingsObject = snapshot.value as? [String: Any] else {
        ...
        if let daysPriorRestriction = snapshot.hasChild("daysPriorRestriction") as? Int {
            self.daysPriorRestriction = daysPriorRestriction
        } else {
            self.daysPriorRestriction = 5
        }

        if let positionsObject = settingsObject["positions"] as? [String: String] {
    ...
    func toFirebaseObject() -> Any {
        var settingsObject: [String: Any] = [
            "teamName": teamName,
            "teamid": teamid,
            "daysPriorRestriction": daysPriorRestriction
        ]
